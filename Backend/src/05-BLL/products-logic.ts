...
async function getOneProduct(id: number): Promise<ProductModel> {
    const sql = "SELECT ProductID AS id, ProductName AS name, UnitPrice AS price, UnitsInStock AS stock FROM Products, CONCAT(ProductID, '.jpg') AS imageName WHERE ProductID = " + id;
    const products = await dal.execute(sql);
